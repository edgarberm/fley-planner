...
final class WidgetGridModel {
    ...
    func setupWidgets(from configs: [DashboardWidgetConfig]) {
        widgets = configs
        ...
        print("✅ Loaded \(widgets.count) widgets from configs")
    }
    
    // Configurar para persistencia
    func configure(userId: UUID, dataService: DataService) {
        self.currentUserId = userId
        self.dataService = dataService
    }
    
    // Guardar posiciones en Supabase
    func saveWidgetPositions() async {
        guard let userId = currentUserId,
              let dataService = dataService else {
            print("❌ Cannot save: missing userId or dataService")
            return
        }
        
        print("💾 Saving widget positions...")
        
        // Convertir widgets a configs
        let configs = widgets.enumerated().map { index, widget in
        ...
        // Guardar en DB
        do {
            try await dataService.saveWidgetConfigs(configs)
            print("✅ Widget positions saved")
        } catch {
            print("❌ Failed to save widget positions: \(error)")
        }
    }
